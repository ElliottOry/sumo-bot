...
def ir_callback(data, addr, _):
    #print(switch.read_u16() >= 5000)
    #print("IR")
    print(switch.read_u16())
    if(switch.read_u16() < 5000):
        #print("NONE IR")
        #print("IR")
        return
        pass
    ...
    if(last != None):
        ...
        if (data==1):
            ...
            motor_on(0, 43690, "forward")
            motor_on(1, 43690, "forward")
        elif (data==2):
            motor_on(0, 43690, "backward")
            motor_on(1, 43690, "backward")
        elif (data==3):
            motor_on(0, 10921, "forward")
            motor_on(1, 10921, "backward")
        elif (data==4):
            motor_on(0, 10921, "forward")
            motor_on(1, 10921, "backward")
        else:
...
buzzer_pin = PWM(16, freq=500, duty_ns=5000)
buzzer_pin.deinit()
low_bat = False
#---------------------------main while--------------------------
...
def loop():
    ...
    while True:
        ...
        voltage = sensor_value * (6.5 / 65535)
        #print(voltage)
...
#was pin 7, 6
i2c = I2C(1, scl=Pin(9), sda=Pin(8), freq=400000)  # Adjust pins if necessary
